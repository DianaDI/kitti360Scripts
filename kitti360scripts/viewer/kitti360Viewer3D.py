...
from kitti360scripts.helpers.labels import name2label, id2label, kittiId2label, id2name, kittiId2name
...
class Kitti360Viewer3D(object):
    ...
    def __init__(self, seq=0, showStatic=True):
        ...
        #     kitti360Path = "C:/Users/Diana/Desktop/DATA/"

        kitti360Path = "C:\\Users\\Diana\\Desktop\\DATA\\Kitti360"
        ...
        # to compute whole dataset stats
        self.num_total_points = 0
        self.semantic_dict = dict(zip(id2label, np.zeros((len(id2label)))))
        self.instance_count_dict = dict(zip(id2label, np.zeros((len(id2label)))))
        self.instance_dict = dict((k, []) for k in id2label)
        self.get_stats(kitti360Path)

    def get_stats(self, root_path,
                  pattern="\\data_3d_semantics\\train\\*\\static\\*.ply"):  # todo: get GT for test data (see dataset docs)
        ...
        for pcdFile in tqdm(pcds):
            ...
            self.counts_semantic_points(pcd_p)
            self.count_instances(pcd_p)
        self.show_stats_semantic()
        self.show_stats_instances()

    def counts_semantic_points(self, pc):
        data = np.asarray(pc['vertex']['semantic'])
        self.num_total_points += len(data)
        for i in data:
            self.semantic_dict[i] += 1

    def count_instances(self, pc):
        data = pc['vertex']
        for vertex in data:
            semantic = vertex['semantic']
            instance = vertex['instance']
            self.instance_dict[semantic].append(instance)

    def sort_dict_by_val(self, d):
        return dict(sorted(d.items(), key=lambda item: item[1], reverse=True))

    def show_stats_instances(self):
        for k, v in self.instance_dict.items():
            self.instance_count_dict[k] = len(set(v))
        total = sum(self.instance_count_dict.values())
        print(f"TOTAL NUM OF INSTANCES: {total}")
        print(self.instance_count_dict)
        # convert id dict to name dict
        instance_count_dict = self.sort_dict_by_val(self.instance_count_dict)
        instance_count_dict_named = {id2name[i]: instance_count_dict[i] for i in instance_count_dict}
        print(instance_count_dict_named)
        self.plot_barplot(instance_count_dict_named, 'kitti360_instances.png', mode="instances")

    def show_stats_semantic(self):
        print(f"TOTAL NUM OF POINTS--: {self.num_total_points}")
        print(self.semantic_dict)
        # convert id dict to name dict
        semantic_dict = self.sort_dict_by_val(self.semantic_dict)
        named_semantic_dict = {id2name[i]: semantic_dict[i] for i in semantic_dict}
        print(named_semantic_dict)
        self.plot_barplot(named_semantic_dict, './kitti360_all.png')

    def plot_barplot(self, dict, name_to_save="fig.png", top_n=20, mode="semantic"):
        total_sum = sum(dict.values())
        print(f"TOTAL NUM: {total_sum}")
        print(dict)
        ...
        plt.tight_layout()
        plt.title(f"KITTI360 {mode} labels")
