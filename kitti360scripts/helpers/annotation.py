    ...
    def generateMeshes(self):
        ...
        if self.vertices_proj:
            for fidx in range(self.faces.shape[0]):
                self.meshes.append(
                    [Point(self.vertices_proj[0][int(x)], self.vertices_proj[1][int(x)]) for x in self.faces[fidx]])
...
class Annotation2D:
    ...
    # Load instance id 
    def loadInstance(self, imgPath, gtType='instance', toImg=True, contourType='instance', semanticCt=True,
                     instanceCt=True):
...
class Annotation3DPly:
    ...
    def __init__(self, labelDir='', sequence='', isLabeled=True, isDynamic=False, showStatic=True, single_file=False):

        if isLabeled and not isDynamic:
            # x y z r g b semanticId instanceId isVisible
            self.fmt = '=fffBBBB'  # '=fffBBBiiBi'  # '=fffBBBiiB'
            self.fmt_len = 16  # 28  # 24
